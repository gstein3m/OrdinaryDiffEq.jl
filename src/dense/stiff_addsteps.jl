...
function _ode_addsteps!(k, t, uprev, u, dt, f, p, cache::Rosenbrock5Cache{<:Array},
    ...
    if length(k) < 3 || always_calc_begin
        ...
        if mass_matrix === I
            @inbounds @simd ivdep for i in eachindex(u)
                linsolve_tmp[i] = du[i] + (dtC62 * k2[i] + dtC64 * k4[i] + dtC61 * k1[i] +
                                   dtC63 * k3[i] + dtC65 * k5[i])
            end
        else
            @inbounds @simd ivdep for i in eachindex(u)
                du1[i] = dtC62 * k2[i] + dtC64 * k4[i] + dtC61 * k1[i] + dtC63 * k3[i] +
                         dtC65 * k5[i]
            end
            mul!(du2, mass_matrix, du1)
            @inbounds @simd ivdep for i in eachindex(u)
        ...
        if mass_matrix === I
            @inbounds @simd ivdep for i in eachindex(u)
                linsolve_tmp[i] = du[i] + (dtC72 * k2[i] + dtC74 * k4[i] + dtC71 * k1[i] +
                                   dtC73 * k3[i] + dtC75 * k5[i] + dtC76 * k6[i])
            end
        else
            @inbounds @simd ivdep for i in eachindex(u)
                du1[i] = dtC72 * k2[i] + dtC74 * k4[i] + dtC71 * k1[i] + dtC73 * k3[i] +
        ...
        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = h31 * k1[i] + h32 * k2[i] + h33 * k3[i] + h34 * k4[i] + h35 * k5[i] +
                     h36 * k6[i] + h37 * k7[i] + h38 * k8[i]
        end
        copyat_or_push!(k, 2, copy(tmp))

        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = h41 * k1[i] + h42 * k2[i] + h43 * k3[i] + h44 * k4[i] + h45 * k5[i] +
                     h46 * k6[i] + h47 * k7[i] + h48 * k8[i]
        end
        copyat_or_push!(k, 3, copy(tmp))
    end
    nothing
